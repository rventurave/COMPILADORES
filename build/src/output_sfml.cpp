...
void run_c_program_simulation() {
    ...
    pushStackFrame();
    {
        ...
        recordStep("Declaring: int* ptrP = (&a)", COLOR_VARIABLE_DECL);
        int* ptrP = (&a);
        std::ostringstream oss_ptrP;
        oss_ptrP << ptrP;
        updateStackFrame("ptrP", oss_ptrP.str());
        recordStep("Declaring: int* ptrQ = (&b)", COLOR_VARIABLE_DECL);
        int* ptrQ = (&b);
        std::ostringstream oss_ptrQ;
        oss_ptrQ << ptrQ;
        updateStackFrame("ptrQ", oss_ptrQ.str());
        recordStep("Assigning to c = *ptrP (valor: " + std::to_string(*ptrP) + ")", COLOR_ASSIGNMENT);
        c = (*ptrP);
        updateStackFrame("c", std::to_string((*ptrP)));
        std::ostringstream _oss_ptrQ;
        _oss_ptrQ<< static_cast<const void*>(ptrQ);
        recordStep("Assigning to ptrP = ptrQ (valor: " +_oss_ptrQ.str()+ ")", COLOR_ASSIGNMENT);
        ptrP = ptrQ;
        updateStackFrame("ptrP", _oss_ptrQ.str());
